...
class KDSMixin(SDE, ABC):
    ...
    def fit(
        ...
        # 1-device mesh + replicated sharding
        mesh = Mesh(onp.array([device]), axis_names=("d",))
        ...
        # init dataloader
        key, subk = random.split(key)
        train_loader = make_dataloader(seed=int(subk[0]), device=device, x=x, batch_size=batch_size)
