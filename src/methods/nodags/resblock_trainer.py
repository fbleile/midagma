...
from src.methods.nodags.functions import indMLPFunction, linearFunction, nonlinearMLP, factorMLPFunction, gumbelSoftMLP
from src.methods.nodags.resblock import iResBlock
from src.methods.nodags.layers.mlpLipschitz import linearLipschitz
from src.methods.nodags.utils.data import experimentDataset
from src.methods.nodags.utils.utils import *

from src.utils.notreks import TrekRegularizer, trek_value

# new helper function for notreks
...
def _ensure_torch_like(W_adj, *, ref: torch.Tensor) -> torch.Tensor:
    """
    Convert W_adj to a torch.Tensor on the same device/dtype as ref.
    Accepts torch / numpy / jax arrays or Python lists.
    """
    if isinstance(W_adj, torch.Tensor):
        return W_adj.to(device=ref.device, dtype=ref.dtype)

    # numpy / list / tuple
    W_np = np.asarray(W_adj)
    return torch.as_tensor(W_np, device=ref.device, dtype=ref.dtype)
...
def compute_loss(
    ...
    x,
    intervention_set,
    l1_regularize: bool = False,
    lambda_c: float = 1e-2,
    fun_type=None,
    trek_reg: Optional["TrekRegularizer"] = None,
):
    # --- make W_adj torch, aligned with x/model ---
    if not isinstance(x, torch.Tensor):
        raise TypeError("compute_loss expects x to be a torch.Tensor")
    W_adj = _ensure_torch_like(W_adj, ref=x)

    e, logdetgrad = model(x, intervention_set, logdet=True)

    observed_set = np.setdiff1d(np.arange(x.shape[1]), intervention_set)
    # model.var should be torch; observed_set is numpy indices (ok)
    lat_var = torch.exp(model.var[observed_set])

    logpe = standard_normal_logprob(
        e[:, observed_set],
        noise_scales=lat_var,
    ).sum(1, keepdim=True)
    ...
    if l1_regularize:
        if fun_type in ("fac-mlp", "gst-mlp"):
            l1_norm = model.f.get_w_adj().abs().sum()
        else:
            l1_norm = sum(p.abs().sum() for p in model.parameters())
        loss = loss + (lambda_c * l1_norm)

    trek_val = None
    if trek_reg is not None and trek_reg.enabled():
        trek_val = trek_value(W_adj, trek_reg)

        if getattr(trek_reg, "mode", "opt") == "opt":
            # keep weight torch-typed; do NOT float() it
            w = torch.as_tensor(
                getattr(trek_reg, "weight", 1.0),
                device=loss.device,
                dtype=loss.dtype,
            )
            loss = loss + w * trek_val

    return loss, torch.mean(logpe), torch.mean(logdetgrad), trek_val
...
class resflow_train_test_wrapper:
    ...
    def train(self, datasets, intervention_sets, return_time=False, return_loss=False, batch_size=64, trek_reg: Optional[TrekRegularizer] = None):
        # training
        ...
        for epoch in range(self.epochs):
            for exp, dataloader in enumerate(train_dataloader):
                for batch, x in enumerate(dataloader):
                    ...
                    loss, logpe, logdetgrad, trek_val = compute_loss(
                        self.model,
                        self.get_adjacency(),
                        x,
                        intervention_set,
